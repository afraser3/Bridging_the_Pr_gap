...
Usage:
    plot_xy_slices_hydro.py [options]
...
Options:
    ...
    --out_name=<str>     Name of figure output directory & base name of saved figures [default: snapshots_xy]
...
for field, cmap in zip(['u', 'v', 'w', 'C', 'T', 'b'], colors):
    if field=='b':
        plotter.add_colormesh(field + '_zwall', x_basis='x', y_basis='y', remove_x_mean=True, transpose=False, cmap=cmap)
    else:
        plotter.add_colormesh(field + '_zwall', x_basis='x', y_basis='y', transpose=False, cmap=cmap)
